...
var serverResults = 'http://analogues.ciat.cgiar.org/outputs/';     /* 2014-08-03 id:01 modification from the direction 'http://analogues.ciat.cgiar.org/outputs/' to local '/analogues/outputs/' */
...
if (isIE)
    alert('Your browser does not support all of the capabilities supply by this web site.\n\nPlease update your browser or use a newest version of Chrome, Safari or Firefox');
...
function stepOne(e) {
    ...
    //addition to validate click on water
     geocoder = new google.maps.Geocoder();
    var latlang = new google.maps.LatLng(y,x);
    geocoder.geocode ({'latLng': latlang},function(results,status){ console.log('status '+results); georesp = status;
        if(status === google.maps.GeocoderStatus.ZERO_RESULTS){
            georesp = status;
              
            dojo.byId('errMsg').innerHTML = '<h4 class="alert-heading">Warning in coordinates</h4> \
                            			<p>No climate data found, please select another site.</p>';
    ...
    console.log('respuesta ', georesp);
    if (georesp === "ZERO_RESULTS"){
        //alert('valida');
        dojo.byId('errMsg').innerHTML = '<h4 class="alert-heading">Warning in coordinates</h4> \
                            			<p>No climate data found, please select another site.</p>';
        dojo.query('#alertMsg').addClass('alert-error');
    ...
    //Add 2015-01-28 Validacion para los paises grandes puedan correr con resolucion 2_5 arc minute
    if(source == '24'/*Brazil*/ || source == '142' /*Russia*/ || source == '185'/*United States*/){
        dojo.byId('refResolution').options[1].removeAttribute('disabled');
        dojo.byId('refResolution').selectedIndex=1;
    }
    if(target == '24'/*Brazil*/ || target == '142' /*Russia*/ || target == '185'/*United States*/){
        dojo.byId('targetResolution').options[1].removeAttribute('disabled');
        dojo.byId('targetResolution').selectedIndex=1;
    }
}
/*
...
function stepThree(e) {
    ...
    var tmean = dojo.byId('tmean').value;
    var prec = dojo.byId('prec').value;
    var rotation = dojo.byId('lng').value;
    ...
    //2014-05-13 Validación para frenar el RunAnalogues si no tiene toda las variables 
...
function updateMapSize() {
    if (map)
        map.getOLMap().baseLayer.onMapResize();
    if (mapResults)
        mapResults.baseLayer.onMapResize();
}
...
function checkClimaticZones() {
    ...
    if (ref == "global" || ref == "africa" || ref == "asia" || ref == "australia" || ref == "europe" ||
        ...
        dojo.byId('refResolution').selectedIndex = 2;
        dojo.byId('refResolution').options[0].setAttribute('disabled');
    } else {
        ...
        dojo.byId('refResolution').options[2].setAttribute('disabled', 'disabled');
    }
    ...
    if (target == "global" || target == "africa" || target == "asia" || target == "australia" || target == "europe" ||
        ...
        dojo.byId('targetResolution').selectedIndex = 2;
        ...
        dojo.byId('targetResolution').options[2].setAttribute('disabled', 'disabled');
    }
}
...
function changeClimaticVars() {
    ...
    dojo.forEach(dojo.query('#divclimaticvars input'), function(i)
    {
        if (i.type == "checkbox" && i.checked == true) {
            ...
            /*if (id !== 'chk_tmean' && id !== 'chk_prec') {
                //dojo.byId('rotationOpts').style.visibility = "hidden";                
            } 
            else if (id === 'chk_tmean' || id === 'chk_prec') {
                    //dojo.byId('rotationOpts').style.visibility = "visible";                
            }*/
            sum += parseFloat(val);
    ...
    //The next two if, are for validation purpose to enable o disable the rotation and temporal scope, when there are not tmean or prec selected
    if(dojo.byId('chk_tmean').checked === false && dojo.byId('chk_prec').checked === false){
        dojo.byId('precbtn').setAttribute('disabled', 'disabled');
        dojo.byId('tmeanbtn').setAttribute('disabled', 'disabled');
        dojo.byId('bothbtn').setAttribute('disabled', 'disabled');
        dojo.byId('nonebtn').setAttribute('disabled', 'disabled');
        dojo.byId('growingSeason1_startDate').value = '';
        dojo.byId('growingSeason1_startDate').setAttribute('disabled', 'disabled');
        dojo.byId('growingSeason1_endDate').value = '';
        dojo.byId('growingSeason1_endDate').setAttribute('disabled', 'disabled');
        dojo.byId('growingSeason2_startDate').value = '';
        dojo.byId('growingSeason2_startDate').setAttribute('disabled', 'disabled');
        dojo.byId('growingSeason2_endDate').value = '';
        dojo.byId('growingSeason2_endDate').setAttribute('disabled', 'disabled');
    }
    else if(dojo.byId('chk_tmean').checked === true || dojo.byId('chk_prec').checked === true){
        dojo.byId('precbtn').removeAttribute('disabled', 'disabled');
        dojo.byId('tmeanbtn').removeAttribute('disabled', 'disabled');
        dojo.byId('bothbtn').removeAttribute('disabled', 'disabled');
        dojo.byId('nonebtn').removeAttribute('disabled', 'disabled');
        dojo.byId('growingSeason1_startDate').value = '1';
        dojo.byId('growingSeason1_startDate').removeAttribute('disabled', 'disabled');
        dojo.byId('growingSeason1_endDate').value = '12';
        dojo.byId('growingSeason1_endDate').removeAttribute('disabled', 'disabled');
        dojo.byId('growingSeason2_startDate').value = '';
        dojo.byId('growingSeason2_startDate').removeAttribute('disabled', 'disabled');
        dojo.byId('growingSeason2_endDate').value = '';
        dojo.byId('growingSeason2_endDate').removeAttribute('disabled', 'disabled');
    }
...
function updateParams() {
    ...
    if (refgcm)
        for (var i = 0; i < refgcm.length; i++)
            analoguesParams.model.push(refgcm[i].toLowerCase());
    if (targetgcm)
        for (var i = 0; i < targetgcm.length; i++)
            analoguesParams.model.push(targetgcm[i].toLowerCase());
...
function showOutput(response) {
    ...
    if (tileLayerName)
        addLayerResultstoMap(tileLayerName);
    //Add result to Map Gallery
    if (response)
        addLastResultstoMapGallery(tileLayerName);
...
function addLayerResultstoMap(lyrName) {
    ...
    climateSimLyr = new OpenLayers.Layer.TMS(lyrName, [ "/tiles/"+lyrName+"/"], {type: 'png', getURL: getAnaloguesMap_url,
...
function initTooltips() {
    ...
    //Resolution
    var resolutionhelp = "<p>Spatial resolution of the dataset. 10 arc-minutes is approximately 19 km when measured at the equator, 2.5 arc-minutes is approximately 5 km,  while 30 arc-seconds is approximately 1 km.</p>";
...
function showMoreInfo(lyrName) {
    ...
    if (prms) {
        ...
        content += '<tr><td><b>Number of time steps</b>:</td><td>12</td></tr>' + //prms.getNdivisions()
                ...
                '<tr><td><b>Threshold </b>:</td><td>' + prms.threshold + ' </td></tr></tbody></table>';
...
function onFullScreenExit() {
    if (logoCCAFSfullScreen)
        dojo.byId('logoCCAFSfullScreen').style.display = "none";
    if (mapResults) {
...
function saveSession() {
    ...
    if (dojo.isFF) {
        ...
        while (downloadLink)
        {
            ...
            {
                if (downloadLink.target == "_blank") {
                    window.open(downloadLink.href, downloadLink.target);
                }
                else {
...
function loadSession() {
    ...
    if (window.FileReader) {
        ...
        fileReader.onload = function(fileLoadedEvent) {
            ...
            if (runs)
                loadandDisplaysessionResults(runs);
        };
...
function showLastResultsSites() {
    getAnaloguesHistoryRuns("http://analogues.ciat.cgiar.org/climate/config/AnaloguesHistoryRuns.csv")
            .then(function(res) {
                //Setup the pager
                res.sort();
                res.reverse();
...
function addLastResultstoMapGallery(lyrName) {
    ...
    if (analoguesHistoryRuns)
        analoguesHistoryRuns.unshift(mr);
...
function setupPager(pageMin, pageMax) {
    ...
    if (analoguesHistoryRuns)
        number_of_items = analoguesHistoryRuns.length || 0;
...
function updateLastResultContainer(pageNumber) {
    ...
    if (analoguesHistoryRuns && (to > analoguesHistoryRuns.length))
        to = analoguesHistoryRuns.length
    var analoguesContainer = dojo.byId("lastvisitedSites");
